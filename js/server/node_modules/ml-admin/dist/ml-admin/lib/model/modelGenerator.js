                                ...
                                case 'mlExtension':
                                    ...
                                    s.visitClassConstructor(cc, {
                                        onImplement: function (impl) {
                                            if (impl.name === 'Extension' && impl.constructorParent.name === 'ml-admin/dist/lib/extensions/extension') {
                                                isValid = true;
                                            }
                                        },
                                    ...
                                    if (!isValid) {
                                        throw 'A class annotated as a MarkLogic extension should implement ml-admin.Extension, at: ' + module.name + ':' + cc.name;
                                    }
                                    var code = "var ExtensionClass = require(" + toModuleName(module.name) + ")." + cc.name + ";\nvar extensionObject = new ExtensionClass();\n";
                                ...
                                onMember: function (member) {
                                    return {
                                        onMemberDecorator: function (decorator) {
                                            switch (decorator.decoratorType.name) {
                                                case 'mlRuleSet':
                                                    if (member.type.primitiveTypeKind !== s.PrimitiveTypeKind.STRING) {
                                                        throw 'A class member annotated as a MarkLogic rule set must be a string property, at: ' + module.name + ':' + cc.name + ':' + member.name;
                                                    }
                                                    ...
                                                    assetModel.ruleSets.push({
                                                        path: s.expressionToLiteral(decorator.parameters[0]).path,
                                                        rules: rules
                                                    });
                                                    break;
                                                case 'mlAlert':
                                                    if (cc.staticType.calls.length === 1 && cc.staticType.calls[0].parameters.length > 0) {
                                                        throw 'A class annotated with a MarkLogic task must have a zero arg constructor, at: ' + module.name + ':' + cc.name + ':' + member.name;
                                                    }
                                                    if (member.type.typeKind !== s.TypeKind.FUNCTION || member.type.parameters.length > 0) {
                                                        throw 'A class member annotated as a MarkLogic task must be a method with zero parameters, at: ' + module.name + ':' + cc.name + ':' + member.name;
                                                    }
                                                    ...
                                                    assetModel.modules[alertModuleName] = {
                                                        ...
                                                        code: "var AlertClass = require(" + toModuleName(module.name) + ")." + cc.name + ";\nvar alertObject = new AlertClass();\nalertObject." + member.name + "();"
                                                    };
                                                case 'mlTask':
                                                    if (cc.staticType.calls.length === 1 && cc.staticType.calls[0].parameters.length > 0) {
                                                        throw 'A class annotated with a MarkLogic task must have a zero arg constructor, at: ' + module.name + ':' + cc.name + ':' + member.name;
                                                    ...
                                                    var taskName = taskOptions.name || member.name;
                                                    ...
                                                    assetModel.modules[taskModuleName] = {
                                                        name: taskModuleName,
                                                        code: "var TaskClass = require(" + toModuleName(module.name) + ")." + cc.name + ";\nvar taskObject = new TaskClass();\ntaskObject." + member.name + "();"
                                                    };
                                            }
                                        }
                                    };
...
function addModules(assetModel, packageDir, modulePaths) {
    ...
    modulePaths.forEach(function (moduleToDeploy) {
        var fileName = path.join(packageDir, moduleToDeploy);
