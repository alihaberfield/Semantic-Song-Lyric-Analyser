...
function generateRawModules(p, rootDir, relativePrefix) {
    ...
    function processSourceFile(sf) {
        ...
        ts.forEachChild(sf, function (node) {
            if (isExported(node) || node.kind === 216) {
                isModule = true;
            }
        });
        if (isModule) {
    ...
    function processExport(typeContainer, node) {
        switch (node.kind) {
            case 181:
                var vars = node;
                var valueKind = vars.declarationList.flags === 8192 ? s.ValueKind.CONST : (vars.declarationList.flags === 4096 ? s.ValueKind.LET : s.ValueKind.VAR);
            ...
            if (symbol && symbol.declarations) {
                symbol.declarations.forEach(function (dec) {
                    if (dec.kind === 216) {
                        ...
                        if (dec.kind !== 139) {
                            var ref = getReference(dec, dec.kind === 206);
            ...
            while (node.kind !== 228) {
                switch (node.kind) {
                    case 206:
                        ...
                        if (node.parent.kind === 228 && globals.modules[mod.name.text]) {
                            moduleName = name_2;
                            break;
                        }
                }
    ...
    function processTypeNode(node, typeNode) {
        var isTypeQuery = (typeNode && typeNode.kind === 145) || node.kind === 145;
        var type;
        if (isTypeQuery) {
            ...
            if (type_2.typeParameters) {
                var typeArgNode = (typeNode || node).kind === 65 ? (typeNode || node).parent : (typeNode || node);
                if (typeArgNode.kind === 147) {
    ...
    function processSignatureType(declaration) {
        ...
        var rawFunctionType = {
            ...
            parameters: params,
            type: (!declaration.type || declaration.type.kind === 99) ? undefined : processTypeNode(declaration.type)
        };
    ...
    function populateClass(parent, clsDec) {
        ...
        if (clsDec.heritageClauses) {
            clsDec.heritageClauses.forEach(function (heritageClause) {
                ...
                else if (heritageClause.token === 79) {
        ...
        clsDec.members.forEach(function (member) {
            var name;
            var type;
            switch (member.kind) {
                case 141:
    ...
    function processDecorators(node, schema) {
        ...
        if (node.decorators) {
            node.decorators.forEach(function (decorator) {
                if (decorator.expression.kind === 65) {
                ...
                else if (decorator.expression.kind === 158) {
...
function isExported(node) {
    ...
    for (var i = 0; i < children.length; i++) {
        if (children[i].kind === 229) {
            var grandChildren = children[i].getChildren();
            for (var j = 0; j < grandChildren.length; j++) {
                if (grandChildren[j].kind === 78) {
                    return true;
                }
            }
        }
